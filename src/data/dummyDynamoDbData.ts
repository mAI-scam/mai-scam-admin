...
const extractCountry = (detection: any): string => {
...
// Process raw DynamoDB data into our dashboard format
const processRawData = (): DashboardData => {
  // Transform raw data to ScamDetection format
  const transformedDetections: ScamDetection[] = rawDynamoDbData.map(
    (detection) => ({
      id: detection["mai-scam"],
      detection_id: detection.detection_id,
      content_type: detection.content_type as
        | "website"
        | "email"
        | "socialmedia",
  ...
  const regionalInsights: RegionalInsight[] = Array.from(countryMap.entries())
    .map(([country, data]) => ({
      country,
      flag:
        country === "Malaysia"
  ...
  // Mock threat trends (would need historical data for real trends)
  const threatTrends: ThreatTrend[] = [
    { date: "2025-01-01", websites: 1, emails: 0, socialMedia: 0, total: 1 },
    { date: "2025-01-02", websites: 1, emails: 1, socialMedia: 1, total: 3 },
    { date: "2025-01-03", websites: 1, emails: 1, socialMedia: 1, total: 3 },
    { date: "2025-01-04", websites: 2, emails: 1, socialMedia: 1, total: 4 },
    { date: "2025-01-05", websites: 2, emails: 1, socialMedia: 1, total: 4 },
    { date: "2025-01-06", websites: 2, emails: 1, socialMedia: 1, total: 4 },
    { date: "2025-01-07", websites: 2, emails: 1, socialMedia: 1, total: 4 },
  ];
...
export const dummyDashboardData: DashboardData = processRawData();
